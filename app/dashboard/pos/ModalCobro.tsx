...
import { X, CreditCard, Banknote, ArrowRightLeft, CheckCircle } from 'lucide-react'
...
interface Props {
  carrito: ItemCarrito[]
  onCerrar: () => void
  onConfirmar: (medioPago: string, tipoComprobante: string, descuentoPct: number) => Promise<void>
}
...
export default function ModalCobro({ carrito, onCerrar, onConfirmar }: Props) {
  ...
  // IVA extraído del total final (para facturas)
  const ivaIncluido = carrito.reduce((acc, i) => {
  ...
  async function handleConfirmar() {
    setProcesando(true)
    await onConfirmar(medioPago, tipoComprobante, descuentoPct)
    setExito(true)
    setProcesando(false)
