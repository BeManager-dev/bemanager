...
export default function CajaPage() {
  ...
  const [nuevoEgreso, setNuevoEgreso] = useState({ descripcion: '', monto: 0, categoria: 'gasto_general' })

  const categorias = [
    { id: 'gasto_general', label: 'Gasto general'      },
    { id: 'retiro',        label: 'Retiro de efectivo' },
    { id: 'cadete',        label: 'Cadete / envio'     },
  ...
  async function cargarDatos() {
    ...
    // Historial (últimas 30 sesiones cerradas)
  ...
  async function cerrarCaja() {
    ...
    await supabase.from('sesiones_caja').update({
      ...
      monto_final_declarado: montoCierre,
  ...
  async function agregarEgreso() {
    ...
    setNuevoEgreso({ descripcion: '', monto: 0, categoria: 'gasto_general' })
            ...
            <div className="bg-white rounded-xl border border-[#E2E8F0] p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-[#64748B]">Monto inicial</span>
                  <span className="text-[#0F172A]">${resumenEfectivo.monto_inicial.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#64748B]">Ventas en efectivo</span>
                  <span className="text-green-600">+${resumenEfectivo.ventas_efectivo.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#64748B]">Egresos</span>
                  <span className="text-red-500">-${resumenEfectivo.egresos.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                </div>
                    ...
                    <div key={e.id} className="flex items-center gap-3 px-5 py-3">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-[#94A3B8]">
                          {categorias.find(c => c.id === e.categoria)?.label} - {new Date(e.created_at).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </div>
                  ...
                  <button onClick={() => verDetalleHistorial(h.id)}
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {h.diferencia !== null && (
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                            ...
                            {h.diferencia > 0 ? '+' : ''}{h.diferencia === 0 ? 'Sin diferencia' : `$${h.diferencia.toLocaleString('es-AR', { minimumFractionDigits: 2 })}`}
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-[#94A3B8]">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium text-[#00B4D8]">
                        ${(h.monto_final_sistema || 0).toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                      </p>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="text-[#0F172A]">${(h.monto_inicial || 0).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span className="text-[#64748B]">Sistema</span>
                              <span className="text-[#0F172A]">${(h.monto_final_sistema || 0).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span className="text-[#64748B]">Declarado</span>
                              <span className="text-[#0F172A]">${(h.monto_final_declarado || 0).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                            </div>
                          </div>
                        </div>
                        <div className="space-y-2">
                          ...
                          {egresosHistorial[h.id] ? (
                            egresosHistorial[h.id].length === 0 ? (
                              <p className="text-sm text-[#94A3B8]">Sin egresos</p>
                            ) : (
                              <div className="space-y-1">
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl">
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <select value={nuevoEgreso.categoria} onChange={e => setNuevoEgreso(p => ({ ...p, categoria: e.target.value }))}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" min={0} step={0.01} value={nuevoEgreso.monto}
                    onChange={e => setNuevoEgreso(p => ({ ...p, monto: Number(e.target.value) }))}
                    className="w-full h-10 pl-7 pr-3 rounded-lg border border-[#E2E8F0] text-sm focus:outline-none focus:border-[#00B4D8] focus:ring-1 focus:ring-[#00B4D8]"
                  />
                </div>
              </div>
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="bg-[#F8FAFB] rounded-xl p-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-[#0F172A]">${resumenEfectivo.saldo_efectivo.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                </div>
              </div>
              <div>
